# Adapted from https://github.com/yuh-zha/AlignScore/tree/main
...
class Inferencer:
    def __init__(
        self, model_name, device, max_input_length, batch_size, cache_dir
    ) -> None:
        ...
        if model_name == "flan-t5-large":
            ...
            self.max_input_length = (
                2048 if max_input_length is None else max_input_length
            )
            ...
            if model_name == "roberta-large":
                ...
                self.max_input_length = (
                    512 if max_input_length is None else max_input_length
                )
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                ckpt, use_fast=True, revision="main", token=None, cache_dir=cache_dir
            )
    ...
    def inference_example_batch(self, doc: list, claim: list):
        ...
        for one_doc, one_claim in tqdm(
            zip(doc, claim), desc="Evaluating", total=len(doc)
        ):
    ...
    def inference_per_example(self, doc: str, claim: str):
        ...
        doc_chunks = [
            chunk.replace(" \n ", "\n").strip()
            for chunk in chunks(doc_sents, self.chunk_size)
        ]
    ...
    def fact_check(self, doc, claim):
        ...
        return (
            outputs["max_support_probs"],
            outputs["used_chunks"],
            outputs["support_prob_per_chunk"],
        )
